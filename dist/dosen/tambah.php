...
session_start();
    if (isset($_POST['tambah_dosen'])) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            include '../../config/database.php';
            $query = mysqli_query($kon, "SELECT max(id_dosen) as id_terbesar FROM dosen");
            $ambil= mysqli_fetch_array($query);
            $id_dosen = $ambil['id_terbesar'];
            $id_dosen++;
            //Membuat kode dosen
            $huruf = "D";
            $kode_dosen = $huruf . sprintf("%03s", $id_dosen);
            ...
            if (!empty($foto)){
                if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                    ...
                    $sql="insert into dosen (kode_dosen,nama_dosen,nip,jk,email,no_telp,alamat,foto) values
                    ('$kode_dosen','$nama','$nip','$jk','$email','$no_telp','$alamat','$foto')";
                }
            }else {
            ...
            //Menyimpan ke tabel dosen
            $simpan_dosen=mysqli_query($kon,$sql);
            ...
            if ($simpan_dosen and $simpan_pengguna) {
                mysqli_query($kon,"COMMIT");
                header("Location:../../index.php?page=dosen&add=berhasil");
            }
            else {
                mysqli_query($kon,"ROLLBACK");
                header("Location:../../index.php?page=dosen&add=gagal");
            }
        }
    }
?>
...
<form action="dist/dosen/tambah.php" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-sm-5">
            <div class="form-group">
                ...
                <input type="text"  class="form-control" name="nip" id="nip"  placeholder="Masukan Nomor Induk Pegawai" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1').replace(/^0[^.]/, '0');" required>
    ...
    <div class="row">
        <div class="col-sm-4">
            <button type="submit" name="tambah_dosen" id="Submit" class="btn btn-primary">Daftar</button>
