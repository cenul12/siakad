...
<tr>
    ...
    <td><button id_jadwal="<?php echo $data['id_jadwal'];?>" class="tombol_input_nilai btn btn-success btn-circle" ><i class="fa fa-mouse-pointer"></i> Input Nilai</button></td>
</tr>
...
<script>
    // Event saat pengguna mengklik tombol input nilai
    $('.tombol_input_nilai').on('click',function(){
        var id_jadwal = $(this).attr("id_jadwal");
        $.ajax({
            url: 'dist/nilai/input.php',
            method: 'post',
            data: {id_jadwal:id_jadwal},
            success:function(data){
                $('#tampil_data').html(data);  
                document.getElementById("judul").innerHTML='Input Nilai';
            }
        });
        // Membuka modal
        $('#modal').modal('show');
